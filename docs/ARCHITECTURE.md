...
| `scripts/proof-fork.ts` (`npm run proof:fork`) | Node 22 + local validator | Clones mainnet (DLMM program, real SPACEX, SPCXx, OPENAI, and USDC mints, both real pools), loads `holdfill_orders` into the local validator, and runs seven checks with the order PDA as delegate: a fill at the holder's price (real 1% fee withheld), issuer pause, revoke, below-minimum fill, and issuer fee change on the SPACEX pool; then, on the real OPENAI mint and OpenAI/USDC pool, a price order filled into classic-token USDC and a price the pool cannot pay, refused. Writes `data/proof-fork.json`. |
