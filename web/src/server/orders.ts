  ...
  sigs.forEach((s, i) => {
    ...
    for (const e of [...parser.parseLogs(logs)].reverse()) {
      ...
      if (e.name === "orderCreated" || e.name === "OrderCreated") {
        ...
        const pricePerToken = !spacex && amountIn > 0 ? amountOut / 1e6 / (amountIn / 1e9) : undefined;
        events.push({ signature: s.signature, time, kind: "filled", amountInRaw: str(d.amountIn), amountOutRaw: str(d.amountOut), requiredRaw: str(d.required), gapPct, pricePerToken });
