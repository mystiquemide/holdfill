...
async function main() {
  ...
  cfg.demo = demo;
  if (!demo.mint) {
    demo.mint = (await createReplicaMint(conn, issuer, { label: "demo PreStock", decimals: 9, multiplier: 1, transferFeeBps: 100, name: "Demo PreStock (simulated, devnet)", symbol: "DEMO", uri: "https://holdfill-preview.midelabs.xyz/demo" })).toBase58();
    writeConfig(cfg);
  }
  if (!demo.successor) {
    demo.successor = (await createReplicaMint(conn, issuer, { label: "demo successor", decimals: 8, multiplier: 1, transferFeeBps: 0, name: "Demo successor (simulated, devnet)", symbol: "DEMOx", uri: "https://holdfill-preview.midelabs.xyz/demo" })).toBase58();
    writeConfig(cfg);
  }
