...
const BADGE_GATED = process.env.REPLICA_BADGE_GATED === "1";

async function createReplicaMint(conn: Connection, issuer: Keypair, spec: MintSpec): Promise<PublicKey> {
...
async function mintInventory(conn: Connection, issuer: Keypair, mint: PublicKey, amount: bigint) {
